...
void printMatchingMachine(char lowestChar = 'a', char highestChar = 'z', int totalStates = MAXS)
{
    int maxc = highestChar - lowestChar;
    for(int s = 0; s < totalStates && s < MAXS; s++){
        cout << s << ": ";
        for(int c = 0; c < maxc; c++){
            if(g[s][c] != -1 && g[s][c] != 0)
                cout << (char)(lowestChar + c) << " --> " << g[s][c] << " | ";
        }
        cout << endl;
    }
}

void printFailureFunction(int totalStates)
{
...
int main(){
    ...
    /*keywords.push_back("momento");
    keywords.push_back("monstro");
    keywords.push_back("mento");
    keywords.push_back("tro");
    string text = "amomentotromonstro";*/
    
    int totalStates = buildMatchingMachine(keywords, 'a', 'z');

    cout << "Matching machine:\n";
    printMatchingMachine('a', 'z', totalStates);

    cout << "Failure function:\n";
    printFailureFunction(totalStates);
